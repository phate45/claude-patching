...
/**
 * Emit a JSON event to stdout (JSON mode only — silent in human mode)
 */
...
// ============ Raw Primitives (orchestrator modules) ============

/**
 * Log a message (human mode only — silent in JSON mode)
 * @param {string} msg
 */
function log(msg) {
  if (!isJsonMode) {
...
/**
 * Log an error (human mode: stderr, JSON mode: error event)
 * @param {string} msg
 */
