...
function _importFromLocal(sourceVersion, targetVersion, targetDir) {
  ...
  const targetData = { source, patches };

  // Carry forward customPatches and suppressedPatches from the source
  if (srcData.customPatches) targetData.customPatches = srcData.customPatches;
  if (srcData.suppressedPatches) targetData.suppressedPatches = srcData.suppressedPatches;
...
function _mergeLocalCustomizations(localVersion, targetDir, result) {
  ...
  // Always carry forward customPatches and suppressedPatches — even if no files
  // were merged this round, the metadata must survive for the next version's import.
