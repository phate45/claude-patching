...
function _mergeLocalCustomizations(localVersion, targetDir, result) {
  ...
  const srcData = JSON.parse(fs.readFileSync(path.join(srcDir, 'patches.json'), 'utf8'));
  const customIds = srcData.customPatches;

  if (!customIds || !customIds.length) {
    // No explicit custom list — nothing to merge (safe default)
    return null;
  }
  ...
  for (const fileId of customIds) {
    const findSrc = path.join(srcDir, `${fileId}.find.txt`);
    const replaceSrc = path.join(srcDir, `${fileId}.replace.txt`);
  ...
  // Carry forward the customPatches list
