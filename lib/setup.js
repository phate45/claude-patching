...
const PROMPT_PATCHING_PATH = '/tmp/prompt-patching';
const PROMPT_PATCHING_REPO = 'https://github.com/ykdojo/claude-code-tips.git';
...
class SetupStatus {
  ...
  toJSON() {
    ...
    return {
      ...
      promptPatching: this.promptPatching,
      errors: this.errors,
  ...
  toReport() {
    ...
    // prompt-patching
    if (this.promptPatching) {
      lines.push(`| prompt-patching | ${this.promptPatching.status} | ${this.promptPatching.details} |`);
    }
...
function discoverInstallations(status) {
  ...
  if (installs.native) {
    try {
      const jsBuffer = extractClaudeJs(installs.native.path);
      ...
      status.warnings.push(`Native extraction failed: ${err.message}`);
    }
  }
...
/**
 * Step 2b: Update prompt-patching repo
 */
function updatePromptPatching(status) {
  try {
    if (fs.existsSync(PROMPT_PATCHING_PATH)) {
      const result = spawnSync('git', ['pull'], {
        cwd: PROMPT_PATCHING_PATH,
        encoding: 'utf8',
        timeout: 30000,
      });
      ...
      if (result.status === 0) {
        const output = result.stdout.trim();
        if (output.includes('Already up to date')) {
          status.promptPatching = { status: '✓', details: 'up to date' };
        } else {
          const changes = output.match(/(\d+) files? changed/);
          const detail = changes ? `${changes[1]} files changed` : 'updated';
          status.promptPatching = { status: 'updated', details: detail };
        }
      } else {
        status.promptPatching = { status: '⚠', details: 'git pull failed' };
        status.warnings.push(`prompt-patching update failed: ${result.stderr}`);
      }
    } else {
      const result = spawnSync('git', ['clone', '--depth', '1', PROMPT_PATCHING_REPO, PROMPT_PATCHING_PATH], {
        encoding: 'utf8',
        timeout: 60000,
      });

      if (result.status === 0) {
        status.promptPatching = { status: 'cloned', details: 'fresh clone' };
      } else {
        status.promptPatching = { status: '⚠', details: 'clone failed' };
        status.warnings.push(`prompt-patching clone failed: ${result.stderr}`);
      }
    }
  } catch (err) {
    status.promptPatching = { status: '⚠', details: err.message };
    status.warnings.push(`prompt-patching: ${err.message}`);
  }
}
...
function processBackup(type, install, status) {
  ...
  // For native, we extract JS from the binary
  if (type === 'native') {
    try {
      ...
      status.errors.push(`Native backup failed: ${err.message}`);
      return;
    }
  }
  ...
  if (sourcePatched) {
    // Source is patched — try .bak fallback before giving up
    const bakPath = install.path + '.bak';
    const bakStats = safeStats(bakPath);
    let bakIsClean = false;
    ...
    if (bakStats.exists) {
      ...
      bakIsClean = !isPatched(bakContent);
    }

    if (bakIsClean) {
      // Clean .bak exists — use it as workspace backup
      fs.copyFileSync(bakPath, backupPath);
      status.backups[type] = {
        status: 'created',
        details: `${formatBytes(bakStats.size)} (from .bak — source is patched)`,
      };
    } else if (backupStats.exists) {
      // No clean .bak, but workspace backup exists — validate it
      const existingContent = fs.readFileSync(backupPath, 'utf8');
      if (isPatched(existingContent)) {
        status.backups[type] = {
          status: '⚠',
...
function runSetup(options = {}) {
  ...
  log('Updating prompt-patching reference...');
  updatePromptPatching(status);
