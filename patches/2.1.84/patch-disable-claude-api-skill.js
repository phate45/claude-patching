...
// Match: function FNAME(){z7({name:"claude-api", ... })}
// The name:"claude-api" string is stable across versions.
const pattern = /function ([$\w]+)\(\)\{z7\(\{name:"claude-api",description:"[^"]*",allowedTools:\[[^\]]*\],userInvocable:!0,async getPromptForCommand\([$\w]+\)\{[^}]*return\[\{type:"text",text:[$\w]+\([$\w]+,[$\w]+\)\}\]\}\}\)\}/;
