 ...
 *
 * 2.1.272 change (step 5): the default (non-shimmer) text-line branch dropped the
 * `inverse:V.highlight?.inverse` prop — native now emits only
 * `color,dimColor,underline,children`. The step-5 regex no longer matches that
 * line; the replacement still emits `inverse` (harmless — reads undefined).
 *
 * Usage:
...
// 2.1.260: the outer match loop became `for(const X of Y){ … }` — a `const`
// binding wrapped in a brace block (was a braceless `for(let X of Y)`). The
// autoAccept sibling loop nearby uses `color:"autoAccept"` (a string, not a
// FN(...) call), so anchoring color on `([$\w]+)\(` still selects only the
// keyword-highlight site.
const inputPattern = new RegExp(
  'for\\((?:let|const) ([$\\w]+) of ([$\\w]+)\\)\\{' +
...
const inputReplacement =
  `for(const ${matchIterVar} of ${matchArrayVar}){` +
...
const renderPattern = new RegExp(
  ...
  'underline:\\1\\.highlight\\?\\.underline,children:' +   // 2.1.272: native dropped the inverse: prop here
