...
// Match isDeferredTool by its unique opening: a function whose first
// statement is `if(X.isMcp===!0)return!0;`. Only one function in the
// entire bundle starts this way — isDeferredTool.
//
// We match just the head, not the full body, so this patch is independent
// of whether feature-flag-toggles has already modified the flag check.
//
// Captures:
//   $1 = function name (QW / D0)
//   $2 = parameter name (A / H)
const pattern = /function ([\w$]+)\(([\w$]+)\)\{if\(\2\.isMcp===!0\)return!0;/;

const match = content.match(pattern);

if (!match) {
  output.error('Could not find isDeferredTool function', [
    'Expected: function X(A){if(A.isMcp===!0)return!0;...',
...
// The rest of the function body (MCP gate, ToolSearch exemption, flag
// check, shouldDefer fallback) is left untouched.
const injection =
