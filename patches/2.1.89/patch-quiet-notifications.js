#!/usr/bin/env node
/**
 * Patch to suppress duplicate background agent notifications (2.1.89+)
 *
 ...
 *
 * Changes from 2.1.50:
 * - Patch Point 2 rewritten: im7 now uses peek (j0H) + bulk-dequeue (P0H)
 *   instead of simple dequeue (TP$). Items are dispatched as an array via
 *   H(array) rather than individual args H(value, meta, mode, uuid).
 *   We now filter the bulk-dequeued array to remove suppressed task IDs
 *   before dispatch, instead of checking a single dequeued item.
...
//
// Pattern: FN(taskId,STATE.setAppState,(x)=>({...x,notified:!0})),{data:{retrieval_status:"success",task:await TASKFN(VAR)}}
// Occurs twice: non-blocking and blocking success paths in TaskOutput.call()
//
// We chain a globalThis Set add after the FN() call:
//   FN(...),(globalThis.__taskOutputRead=globalThis.__taskOutputRead||new Set).add(taskId),{data:...}
...
//
// In 2.1.89 the dispatch function was refactored from single-dequeue to bulk:
//   function im7({executeInput:H}){
//     let $=(f)=>f.agentId===void 0, q=j0H($);  // peek with filter
//     if(!q)return{processed:!1};
//     if(nm7(q)||q.mode==="bash"){let f=hiH($);return H([f]),{processed:!0}}
//     let K=q.mode, _=P0H((f)=>$(f)&&!nm7(f)&&f.mode===K);  // bulk dequeue
//     if(_.length===0)return{processed:!1};
//     return H(_),{processed:!0}
//   }
//
// We match the tail of this function — from the bulk-dequeue length check
// through the closing brace — anchored by the `var` that follows the function.
// We inject filtering of `_` when K==="task-notification" before dispatch.
...
const arrayVar = im7TailMatch[2]; // The bulk-dequeued array (_)
const execVar = im7TailMatch[3];  // The executeInput function (H)
...
const modeVar = modeVarMatch[1]; // K
