...
//
// 2.1.153 structure: loadedNestedMemoryPaths is now an object accessed via
// bracket indexing instead of a Set:
//   for(let ITEM of ARG){if(STATE.loadedNestedMemoryPaths?.[ITEM.path])continue;if(!STATE.readFileState.has(ITEM.path)){
//
// Previously (2.1.89): used STATE.loadedNestedMemoryPaths?.has(ITEM.path)
//
// We extend the readFileState condition to also check against the content set.

const site2Pattern = /for\(let ([$\w]+) of [$\w]+\)\{if\(([$\w]+)\.loadedNestedMemoryPaths\?\.\[\1\.path\]\)continue;if\(!\2\.readFileState\.has\(\1\.path\)\)\{/;
const site2Match = content.match(site2Pattern);

if (!site2Match) {
  output.error('Could not find nested traversal readFileState check (site 2)', [
    'Expected: for(let ITEM of ARG){if(STATE.loadedNestedMemoryPaths?.[ITEM.path])continue;if(!STATE.readFileState.has(ITEM.path)){',
