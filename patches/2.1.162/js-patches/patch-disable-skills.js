 ...
 *
 *   let Y=ll([...q,...f,...z,...K,..._,...A,...YI$()]);return ...
 *
...
// distinctive `ll([...spread,...spread,...,...call()])` flatten followed
// immediately by `);return `. Only one site in the bundle matches.
// Captures:
//   $1 = list variable name (Y)
//   $2 = the full ll([...]) array literal, re-emitted verbatim
const pattern = /([\w$]+)=ll\((\[(?:\.\.\.[\w$]+(?:\(\))?,)+\.\.\.[\w$]+(?:\(\))?\])\);return /;
...
const [original, listVar, arrLiteral] = match;
...
// Re-emit `Y=ll([...]);` + filter + `return ` (splice between ; and return,
// keeping the preceding comma-let declaration intact).
const replacement = `${listVar}=ll(${arrLiteral});${filter}return `;
