 ...
 *     Also publishes the gx() bridge — see "The chunk-scope trap" below.
 *  7. SendMessage prompt: $Hp(Vc()) -> $Hp(Vc()&&globalThis.__wfGate?.()), which
 *     drops the whole "Protocol responses (legacy)" block (team-lead,
 ...
 *
 * The chunk-scope trap
 * -------------------
 * Since 2.1.246 the bundle is ~1700 separately-scoped Bun chunk modules, each
 * with its own imports; patches run against the flat concat and are chunk-blind.
 * gx() is captured from the Workflow tool definition (chunk-d0pvfsg4) and is
 * legitimately in scope at points 3/5/6/12/14/15, which all land in the mega
 * chunk (chunk-b2hkftr9) that imports it under the same name.
 *
 * Point 7 does NOT. The SendMessage tool definition lives in its own chunk
 * (chunk-ks5hn3y3) which imports Vc but never gx, so `return $Hp(Vc()&&gx())`
 * compiled to a free variable. `--check` passed (the pattern matched) and the
 * binary loaded, but the moment anything built SendMessage's description —
 * ToolSearch `select:SendMessage`, or a keyword search that ranks it — the
 * builder threw `ReferenceError: gx is not defined`. That surfaced as an empty
 * `{}` tool result, and then every subsequent attachment render died in the
 * try/catch around the reminder renderer, repeating the ReferenceError after
 * every event for the rest of the session.
 *
 * Fix: a globalThis bridge, in the house style of the other patches' knobs
 * (__immTools, __disabledSkills, __abbrevMap). Point 6 publishes gx under
 * `globalThis.__wfGate` from a TOP-LEVEL statement — the per-tool strip Map is a
 * module-init `var`, so the bridge is armed before any runtime call, unlike the
 * function bodies at points 3/5 which only run on demand. Point 7 then reads it
 * with `?.()`: if the bridge ever fails to arm, the gate reads falsy and the
 * teammate vocabulary stays out of the prompt — the patch's own default — rather
 * than throwing.
 *
 * Rule of thumb for future points: an identifier captured at site A may only be
 * injected at site B if both sites live in the same chunk. When they don't,
 * bridge through globalThis from a top-level statement in the chunk that has it.
 *
 * Usage:
...
// The bridge identifier — published at point 5 (top-level, in the chunk that
// owns the gate) and read from chunks that never imported it.
const WF_BRIDGE = 'globalThis.__wfGate';

// In 2.1.280 the dispatch ternary moved into a chunk with no binding for the
// workflow gate, so read it through the bridge rather than injecting the name
// directly. An unarmed bridge yields undefined -> teammates off, which is the
// direction this patch wants anyway.
replaceExact('teammate dispatch gate',
  teamCtxFull,
  `${teamVar}=${swarmGateFn}()&&${WF_BRIDGE}?.()?${stateVar}.teamContext:void 0`);
...
// ── Patch Point 6: keep `name` out of the strip list, and arm the gx() bridge ──
//
...
//
// This `var Fpb = new Map([...])` is a top-level statement in the same chunk
// that imports gx (points 3/5/12/14/15 already inject gx() here, so the binding
// is proven present by every other point in this patch). A comma expression in
// the array element publishes gx to globalThis at module-init time, which is
// what point 7 reads from its own, gx-less chunk. See the header.
...
replaceExact('Agent strip list + workflow-gate bridge',
  stripListMatch[0],
  `[${stripListMatch[1]},(${WF_BRIDGE}=${workflowFn},["team_name","mode"])]`);

// ── Patch Point 7: gate the SendMessage prompt's teams flag ──
//
//   async prompt(){return $Hp(Vc())}
//     ->  async prompt(){return $Hp(Vc()&&globalThis.__wfGate?.())}
// Drops the entire "Protocol responses (legacy)" section (team-lead, shutdown
// and plan-approval frames) and arms points 8/9.
//
// gx() canNOT be called directly here: this site is in the SendMessage chunk,
// which does not import it. Reading the bridge armed at point 6 instead. The
// `?.()` is the safety net — an unarmed bridge reads falsy (teammate prose
// dropped) rather than throwing mid-description. See the header.

replaceExact('SendMessage prompt flag',
  `return ${sendMsgFn}(${swarmGateFn}())`,
  `return ${sendMsgFn}(${swarmGateFn}()&&${WF_BRIDGE}?.())`,
...
// (unrelated) cross-session flag. In 2.1.280 both branches became template
// literals and the fallback branch grew to spell out every accepted form:
// `Recipient: a background agent's name or agentId, a teammate name, or "main"`.
// Only that fallback branch needs the reword — drop the teammate clause and
// leave the cross-session branch alone. Match the literal body (still unique).
...
// ── Patch Point 17: /skillify execution-mode list — RETIRED in 2.1.260 ──
//
// Prior versions gated the `Teammate` execution mode out of the /skillify
// step-annotation list. In 2.1.260 that string lives inside the Skillify prompt
// builder `Cs()` (`# Skillify {{userDescriptionBlock}}`), which is DEAD CODE:
// not exported from its chunk (exports {tae,u2e,d2e,Krt}), no in-chunk caller,
// and no `skillify`/`/skillify` command token anywhere in the bundle. Nothing
// invokes it, so editing that string has zero runtime effect.
//
// Retired rather than ported: it edits unreachable text, and (bonus) it was the
// sole edit landing in chunk 429 — a 213 KB module whose freed bytecode region
// (163 KB) is smaller than the chunk, giving zero growth headroom. Removing the
// point eliminates that reassembly fragility at the source. Re-add a gated
// version only if a future build re-wires Skillify to a live invocation surface.
