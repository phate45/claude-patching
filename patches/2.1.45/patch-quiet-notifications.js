#!/usr/bin/env node
/**
 * Patch to suppress duplicate background agent notifications (2.1.45+)
 *
 ...
 *
 * Changes from 2.1.41:
 * - TaskOutput: task extraction is now async (await FN(VAR))
 * - hD1 consumer: guard sequence changed, task-started filter added
 * - Main loop consumer: regex unchanged
 *
 * Usage:
...
//
// Pattern (2.1.45): nM(D,$.setAppState,(U)=>({...U,notified:!0})),{data:{retrieval_status:"success",task:await gq$(M)}}
// Occurs twice: non-blocking and blocking success paths in TaskOutput.call()
//
// Change from 2.1.41: task extraction is now async (await FN(VAR))
//
// We chain a globalThis Set add after the nM() call:
//   nM(...),(globalThis.__taskOutputRead=globalThis.__taskOutputRead||new Set).add(taskId),{data:...}
...
//
// In 2.1.45 the React useEffect dequeue sequence changed:
//   if(!TVA())return;let M=NVA();if(!M)return;if(M.mode==="task-started")return;f(!0);...
//
// Changes from 2.1.41:
// - Loading guard moved earlier (no longer adjacent to dequeue)
// - New task-started filter after null check
//
// We insert a check after the null-guard and BEFORE the task-started filter,
// so suppressed notifications don't trigger either path.

const hd1ConsumerPattern = /(if\(![$\w]+\(\)\)return;let ([$\w]+)=([$\w]+)\(\);if\(!\2\)return;)(if\(\2\.mode==="task-started"\)return;[$\w]+\(!0\))/;

const hd1Match = content.match(hd1ConsumerPattern);

if (!hd1Match) {
  output.error('Could not find hD1 notification consumer pattern', [
    'Expected: if(!FN())return;let ITEM=FN();if(!ITEM)return;if(ITEM.mode==="task-started")return;SET(!0)',
    'This might be an unsupported Claude Code version'
  ]);
  process.exit(1);
}
...
const hd1ItemVar = hd1Match[2]; // The dequeued item variable (M)
