 ...
 *
 * 2.1.80 change: bUD's simple-path text element switched from a `let`
 * declaration to a React memo-cache `if/else` pattern:
 *   Old: else _=$[3];let w=aI.createElement(E,{color:"text"},A)
 *   New: if($[21]!==A)j=T7.createElement(E,{color:"text"},A),$[21]=A,$[22]=j;else j=$[22]
 *
 * Usage:
...
// 2.1.80+ memo-cache pattern
const renderPatternNew = new RegExp(
  'if\\(\\$\\[(\\d+)\\]!==([$\\w]+)\\)' +         // if($[SLOT1]!==TEXTVAR)
  '([$\\w]+)=([$\\w]+)\\.createElement\\(' +         // ELEM=REACT.createElement(
  '([$\\w]+),\\{color:"text"\\},\\2\\),' +           // TEXT,{color:"text"},TEXTVAR),
  '\\$\\[\\1\\]=\\2,' +                              // $[SLOT1]=TEXTVAR,
  '\\$\\[(\\d+)\\]=\\3;' +                           // $[SLOT2]=ELEM;
  'else \\3=\\$\\[\\6\\]'                            // else ELEM=$[SLOT2]
);

// Pre-2.1.80 let-declaration pattern (fallback)
const renderPatternOld = /else ([$\w]+)=([$\w]+)\[(\d+)\];let ([$\w]+)=([$\w]+)\.createElement\(([$\w]+),\{color:"text"\},([$\w]+)\)/;

let renderMatch = patched.match(renderPatternNew);
let renderMode = 'new';

if (!renderMatch) {
  renderMatch = patched.match(renderPatternOld);
  renderMode = 'old';
}

if (!renderMatch) {
  output.error('Could not find bUD text render pattern', [
    'Expected (2.1.80+): if($[N]!==A)j=R.createElement(T,{color:"text"},A),$[N]=A,$[M]=j;else j=$[M]',
    'Expected (pre-2.1.80): else VAR=ARR[N];let VAR=REACT.createElement(TEXT,{color:"text"},VAR)',
    'The user message text renderer structure may have changed',
  ]);
  process.exit(1);
}

let renderOriginal, renderReplacement;

if (renderMode === 'new') {
  const [original, slot1, textVar, elemVar, reactVar, textComp, slot2] = renderMatch;
  ...
  const [original, ptrVar, cacheArr, cacheIdx, elemVar, reactVar, textComp, textVar] = renderMatch;
  renderOriginal = original;

  renderReplacement =
    `else ${ptrVar}=${cacheArr}[${cacheIdx}];let ${elemVar}=${reactVar}.createElement(${textComp},{color:"text"},` +
    `${textVar}[0]=="\\u23F0"?` +
    `[${reactVar}.createElement(${textComp},{bold:!0},${textVar}.slice(0,11)),${textVar}.slice(11)]:` +
    `${textVar})`;

  output.discovery('bUD text renderer (let-decl)', renderOriginal.slice(0, 80) + '...', {
    'React var': reactVar,
    'Text component': textComp,
    'text var': textVar,
  });
