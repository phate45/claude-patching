 ...
 * In 2.1.92+ the spinner is a memoized closure:
...
// VAR=MEMO(()=>{if(process.env.TERM==="xterm-ghostty")return[...];return[...]},()=>process.env.TERM)
// MEMO is a minified memoizer name (_8, K8, etc.) — captured as group 2
const memoizedPattern = /([$\w]+)=([$\w]+)\(\(\)=>\{if\(process\.env\.TERM==="xterm-ghostty"\)return\["[^"]*(?:","[^"]*)*"\];return\["[^"]*(?:","[^"]*)*"\]\},\(\)=>process\.env\.TERM\)/;
